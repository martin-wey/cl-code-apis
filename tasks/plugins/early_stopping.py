...
class EarlyStoppingPlugin(SupervisedPlugin):
    ...
    def before_training_epoch(self, strategy, **kwargs):
        if self.peval_mode == "epoch":
            ...
            if curr_step - self.best_step >= self.patience:
                if self.verbose:
                    print(f"EarlyStopping: out of patience, stop training for exp",
                          '{:03}'.format(strategy.clock.train_exp_counter))
                strategy.model.load_state_dict(self.best_state)
                strategy.stop_training()
    ...
    def _update_best(self, strategy):
        ...
        if self.criteria == 'exp':
            metric_key = f"{self.metric_key}/Exp" + '{:03}'.format(strategy.clock.train_exp_counter)
        names = [k for k in res.keys() if k.startswith(metric_key)]
        if len(names) == 0:
            return None
