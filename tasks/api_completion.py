...
def generate_api_call_completion_samples(example):
    samples = []
    # split api usages
...
def generate_api_usage_completion_samples(example):
    samples = []
    # split api usages
    api_usages = list(filter(lambda e: e != '', example['api_seq'].split('|')))
    api_usages = [u.split('.') for u in api_usages]
...
def evaluate_api_call_completion(cfg: omegaconf.DictConfig,
                                 model: transformers.AutoModelForCausalLM,
                                 tokenizer: transformers.AutoTokenizer,
                                 dataset: datasets.Dataset):
    def tokenize_function(example):
        return tokenizer(example['source_code'], return_attention_mask=False, add_special_tokens=False)
    ...
    with multiprocessing.Pool(cfg.run.preprocessing_num_workers) as pool:
        results = list(tqdm(pool.imap(generate_api_call_completion_samples, iter(dataset)), total=len(dataset)))
...
def evaluate_api_usage_completion(cfg: omegaconf.DictConfig,
                                  ...
                                  dataset: datasets.Dataset):
    def tokenize_function(example):
        return tokenizer(example['context'], return_attention_mask=False, add_special_tokens=False)
    ...
    logger.info("Generating test samples.")
    with multiprocessing.Pool(cfg.run.preprocessing_num_workers) as pool:
        results = list(tqdm(pool.imap(generate_api_usage_completion_samples, iter(dataset)), total=len(dataset)))
    results = [item for sublist in results for item in sublist]
    dataset = Dataset.from_pandas(pd.DataFrame(results))
